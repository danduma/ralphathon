  ...
  return (
    <main className="screen-page">
      <header className="screen-header">
        <div>
          <p className="eyebrow">Earned Physical Action Harness</p>
          <h1>Can Codex turn on this lamp?</h1>
        </div>
      ...
      <section className="screen-grid">
        ...
        <aside className="control-column">
          ...
          <section className={`lamp-panel ${snapshot.events.some((event) => event.type === "physical_action.fired") ? "fired" : ""}`} aria-label="Lamp trigger status">
            <header className="panel-header">
              <span>Physical action</span>
              <strong>{snapshot.events.some((event) => event.type === "physical_action.fired") ? "fired" : "locked"}</strong>
            </header>
            <div className="lamp-bulb" />
            <p>{snapshot.events.some((event) => event.type === "physical_action.fired") ? "Verified success token released the relay." : "Useful measurable work required before the room responds."}</p>
          </section>
