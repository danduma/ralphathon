...
describe("HapticsManager", () => {
  ...
  it("can prime vibration during a user gesture without firing a race pulse", () => {
    const vibrate = vi.fn(() => true);
    Object.defineProperty(navigator, "vibrate", { configurable: true, value: vibrate });
    const manager = new HapticsManager();
    expect(manager.prime()).toBe("supported");
    expect(vibrate).toHaveBeenCalledWith(20);
    expect(manager.getSnapshot().visualPulseToken).toBe(0);
  });
