...
test("screen and phone clients share the typed race event stream", async ({ browser, page }) => {
  ...
  await expect(page.getByRole("heading", { name: /Watch agents race/ })).toBeVisible();
  await expect(page.getByRole("button", { name: "Run race" })).toBeVisible();
  ...
  for (const [index, phone] of phones.entries()) {
    ...
    await expect(phone.getByRole("heading", { name: "Pick a racer" })).toBeVisible();
  ...
  await expect(phones[0].getByText(/Atlas passes|Atlas rings/)).toBeVisible({ timeout: 12_000 });
  await expect(page.locator(".race-topline")).toContainText("Done", { timeout: 12_000 });
